...
class HummingIndexedExperts(HummingExpertsBase):
    ...
    def apply(
        ...
        tiered = getattr(self, "_qwen38_tiered_prefill", None)
        if tiered is not None and hidden_states.shape[0] > tiered.max_decode_tokens:
            from vllm.model_executor.layers.quantization.compressed_tensors.compressed_tensors_moe.stream_stage import maybe_apply as _stream_apply
            if _stream_apply(self, output=output, hidden_states=hidden_states,
                    topk_weights=topk_weights, topk_ids=topk_ids, activation=activation,
                    expert_map=expert_map, a1q_scale=a1q_scale, a2_scale=a2_scale,
                    workspace13=workspace13, workspace2=workspace2,
                    expert_tokens_meta=expert_tokens_meta,
                    apply_router_weight_on_input=apply_router_weight_on_input):
                return
            from vllm.model_executor.layers.quantization.compressed_tensors.compressed_tensors_moe.tiered_vmm import apply_tiered
            apply_tiered(self, tiered.experts, tiered.cache,
                output=output, hidden_states=hidden_states,
                w1=tiered.w13, w2=tiered.w2,
